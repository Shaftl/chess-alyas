      ...
      (async () => {
        try {
          ...
          const needFetchIds = rawPlayers
            ...
            .filter(Boolean)
            .filter((id) => {
              // If it's a bot id, skip fetching
              if (typeof id === "string" && id.toLowerCase().startsWith("bot:"))
                return false;
              // If it's clearly a socket id (not a 24-hex mongo id), skip fetch
              // We only fetch profiles for real DB user ids (24 hex chars)
